...
palindrome "" = True
palindrome (x:[]) = True
palindrome x = if head x == last x then palindrome(init(tail(x))) else False
...
capitalize s = unwords(map capit (words s))
  where capit (x:xs) = toUpper(x) : xs
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\x -> x <= max) [k^i | i <- [0..]]
...
joinToLength i xs = filter (\x -> length x == i) xs ++ (filter (\x -> length x == i) [x++j | x <- xs, j <- xs])
...
sumRights xs = foldr (+) 0 (map rightmappa xs)

rightmappa :: Either a Int -> Int
rightmappa (Left x) = 0
rightmappa (Right x) = x
...
multiApp f gs x = f (gslist gs x)

gslist [] x = []
gslist (g:gs) x = g x : gslist gs x
...
interpreter commands = interp commands 0 0
  where interp [] x y = []
        interp (c:cs) x y
          | c == "up" = interp cs x (y+1)
          | c == "down" = interp cs x (y-1)
          | c == "right" = interp cs (x+1) y
          | c == "left" = interp cs (x-1) y
          | c == "printX" = show x : interp cs x y
          | c == "printY" = show y : interp cs x y
