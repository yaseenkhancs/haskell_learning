...
leftpad s i = if i > length s then leftpad (" " ++ s) i else s
...
countdown i = "Ready! " ++ cthelper i ++ "Liftoff!"

cthelper 0 = ""
cthelper i = show i ++ "... " ++ cthelper (i-1)
...
smallestDivisor n = smalldivhelper n (n-1)
  where smalldivhelper x y = if x `mod` y == 0 then y else smalldivhelper x (y-1)
--smalldivhelper :: Integer -> Integer -> Integer
...
isPrime x = if smallestDivisor x == 1 then True else False
